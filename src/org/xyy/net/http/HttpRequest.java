 ...
 * 提倡带简单的参数的GET请求采用类似REST风格, 因此提供了 getRequstURIPath 系列接口。  <br>
 * 例如简单的翻页查询   <br>
...
public class HttpRequest extends Request<HttpContext> {
     ...
     * @return Flipper翻页对象
     */
    public org.xyy.source.Flipper getFlipper() {
     ...
     * @return Flipper翻页对象
     */
    public org.xyy.source.Flipper getFlipper(boolean needcreate) {
     ...
     * @return Flipper翻页对象
     */
    public org.xyy.source.Flipper getFlipper(int maxLimit) {
     ...
     * @return Flipper翻页对象
     */
    public org.xyy.source.Flipper getFlipper(boolean needcreate, int maxLimit) {
        return getFlipper("flipper", needcreate, maxLimit);
    }

    /**
     * 获取翻页对象 https://xyy.org/pipes/records/list/offset:0/limit:20/sort:createtime%20ASC  <br>
     * https://xyy.org/pipes/records/list?flipper={'offset':0,'limit':20, 'sort':'createtime ASC'}  <br>
     * 以上两种接口都可以获取到翻页对象
     *
     *
     ...
     * @return Flipper翻页对象
     */
    public org.xyy.source.Flipper getFlipper(String name, boolean needcreate, int maxLimit) {
        org.xyy.source.Flipper flipper = getJsonParameter(org.xyy.source.Flipper.class, name);
        if (flipper == null) {
            if (maxLimit < 1) maxLimit = org.xyy.source.Flipper.DEFAULT_LIMIT;
            int limit = getRequstURIPath("limit:", maxLimit);
            int offset = getRequstURIPath("offset:", 0);
            String sort = getRequstURIPath("sort:", "");
            if (limit > 0) flipper = new org.xyy.source.Flipper(limit, offset, sort);
        ...
        if (maxLimit < 1) maxLimit = org.xyy.source.Flipper.DEFAULT_LIMIT;
        return new org.xyy.source.Flipper(maxLimit);
    }
}
